...
from tkinter import messagebox, scrolledtext  # Se importan los cuadros de diálogo y texto con scroll
...
# Crear las entradas para los valores iniciales (vector x0) con valores predeterminados de 0
entries_x0 = [tk.Entry(main_frame, width=5, justify="center") for _ in range(3)]
for entry in entries_x0:
    entry.insert(0, "0")  # Valor predeterminado: 0
...
solve_button = tk.Button(button_frame, text="Resolver", command=lambda: solve_system(entries, entries_b, entries_x0, tol_entry, result_label), width=10)
solve_button.grid(row=0, column=0, padx=5)

# Botón para restablecer todos los campos
reset_button = tk.Button(button_frame, text="Restablecer", command=lambda: reset_fields(entries, entries_b, entries_x0, tol_entry, result_label), width=10)
